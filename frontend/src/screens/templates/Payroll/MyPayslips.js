...
const MyPayslips = () => {
  ...
  const handleLinkEmployee = async (empId) => {
    try {
      ...
      setError(
        error.response?.data?.message || "Error linking employee account"
      );
    }
  };
  ...
  const verifyPayslipAccess = (payslipId) => {
    ...
    if (!payslip) {
      setError(
        "Invalid payslip selected. Please refresh the page and try again."
      );
      return false;
    }
    return true;
    ...
    try {
      ...
      if (response.data.success) {
        ...
        if (response.data.needsLinking) {
          setError(
            "Your account is not linked to an employee record. Please contact HR to link your account to your employee profile."
          );
        }
      } else {
      ...
      if (error.response?.status === 401) {
        ...
        setError(
          "No employee record found for your account. Please contact HR to link your account."
        );
        ...
        setError(
          error.response?.data?.message ||
            "Error fetching payslips. Please try again."
        );
      }
    } finally {
      setLoading(false);
  ...
  const handleDownloadPayslip = async (payslipId, retryCount = 0) => {
    ...
    try {
      ...
      let filename = `payslip_${employee?.empId || "unknown"}_${payslipId.slice(
        -6
      )}.pdf`;
      ...
      if (error.response && error.response.data instanceof Blob) {
        try {
          ...
          if (error.response.status === 401) {
            if (retryCount === 0) {
              console.log(
                "Authentication failed, clearing auth and redirecting..."
              );
              localStorage.removeItem("token");
              localStorage.removeItem("companyCode");
          ...
          setError(
            "Error downloading payslip - server returned invalid response"
          );
        }
      } else {
        ...
        if (error.response?.status === 403) {
          setError(
            "Access denied. This payslip may not belong to your account or you may not have permission to download it. Please contact HR if you believe this is an error."
          );
        } else if (error.response?.status === 404) {
          setError(
          ...
          setError(
            error.response?.data?.message ||
              "Error downloading payslip. Please try again later."
          );
        }
      }
    } finally {
  ...
  const calculateAllowancesTotal = (payslip) => {
    if (payslip.allowances && Array.isArray(payslip.allowances)) {
      return payslip.allowances.reduce(
  ...
  const calculateDeductionsTotal = (payslip) => {
    if (payslip.deductions && Array.isArray(payslip.deductions)) {
      return payslip.deductions.reduce(
      ...
      >
        ...
        <DialogContent sx={{ p: 0 }}>
          <Box sx={{ p: 3 }}>
            ...
            <Card
              variant="outlined"
              sx={{ mb: 3, bgcolor: alpha(theme.palette.primary.main, 0.02) }}
            >
              <CardContent>
                <Typography
                  variant="h6"
            ...
            <Card
              variant="outlined"
              sx={{ mb: 3, bgcolor: alpha(theme.palette.info.main, 0.02) }}
            >
              <CardContent>
                <Typography
                  variant="h6"
            ...
            {(payslip.lopDays > 0 || payslip.payableDays) && (
              <Card
                variant="outlined"
                sx={{ mt: 3, bgcolor: alpha(theme.palette.warning.main, 0.02) }}
              >
                <CardContent>
                  <Typography
                    variant="h6"
                    ...
                    {payslip.lopDays !== undefined && (
                      <Grid item xs={6} sm={4}>
                        ...
                        <Typography
                          ...
                          fontWeight="600"
                          color={
                            payslip.lopDays > 0 ? "error.main" : "success.main"
                          }
                        >
        ...
        <DialogActions
          sx={{ p: 3, bgcolor: alpha(theme.palette.grey[500], 0.05) }}
        >
          <Button
            onClick={onClose}
            variant="outlined"
          ...
          <Card elevation={2}>
            <CardContent sx={{ p: 3 }}>
              <Box display="flex" alignItems="center" gap={2} mb={3}>
                ...
                <Chip
                  label={`${getMonthName(payslips[0].month)} ${
                    payslips[0].year
                  }`}
                  color="primary"
                  variant="outlined"
                ...
                <Grid item xs={12} md={6}>
                  ...
                  >
                    <CardContent>
                      ...
                      {payslips[0].allowances &&
                      payslips[0].allowances.length > 0 ? (
                ...
                <Grid item xs={12} md={6}>
                  ...
                  >
                    <CardContent>
                      ...
                      payslips[0].deductions.length > 0 ? (
                      ...
                      >
                        ...
                        >
                          {formatCurrency(
                            calculateDeductionsTotal(payslips[0])
                          )}
                        </Typography>
                      </Box>
                    </CardContent>
